...
# Constants
SERIAL_PORT = '/dev/cu.usbmodem1101'  # Arduino's serial port (corrected)
BAUD_RATE = 9600
SEND_SERIAL = True
...
class MainProgram:
    ...
    def handle_triggers(self):
        ...
        left_bumper = self.controller.get_axis(LEFT_BUMPER)
        right_bumper = self.controller.get_axis(RIGHT_BUMPER)


        if abs(left_trigger) > TRIGGER_THRESHOLD and self.claw_position>0:
            self.rotate_claw(left_trigger, -1)
        elif abs(right_trigger) > TRIGGER_THRESHOLD and self.claw_position<180:
            self.rotate_claw(right_trigger, 1)


        if abs(left_bumper) > TRIGGER_THRESHOLD and self.roll_position>0:
            self.adjust_claw(left_bumper, -1)
        elif abs(right_bumper) > TRIGGER_THRESHOLD and self.roll_position<180:
            self.adjust_claw(right_bumper, -1)
       
    def rotate_claw(self, trigger_value, close):
        rotation_speed = trigger_value * (5 * close)  # Adjust multiplier based on desired speed
    ...
    def adjust_claw(self, trigger_value, close):
        target_position = CLAW_CLOSED + ((trigger_value * close) + 1) / 2 * (CLAW_OPEN - CLAW_CLOSED)
