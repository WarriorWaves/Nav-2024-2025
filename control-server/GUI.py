...
class ROVControlPanel(QMainWindow):
   def __init__(self):
       super().__init__()
       self.controller = ROVController(123)
   ...
   def keyPressEvent(self, event):
       ...
       if event.key() == Qt.Key_C:
           self.capture_camera_feed2()


   def capture_camera_feed2(self):
       ret, frame = self.video_feed2.capture.read()
       if ret:
           save_path = "/Users/kashishkapoor/Desktop/captured_image.png"
           cv2.imwrite(save_path, frame)
           print(f"Image saved to to the folerder")
       else:
           print("Failed to capture image from Camera Feed 2.")
